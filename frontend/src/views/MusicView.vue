...
import { Check, Disc3, Download, FolderCog, LoaderCircle, Music2, Pause, Play, RefreshCw, Search } from '@lucide/vue'
...
onMounted(async () => {
  await Promise.all([media.load(), sources.load(), offline.load()])
...
<template>
  <section class="page music-page">
    ...
    <template v-if="libraryMusic.length">
      <label class="search-box"><Search :size="18" /><input v-model="query" :placeholder="t('music.search')" /></label>
      <section class="album-section"><div class="section-heading"><h2>{{ t('music.albums') }}</h2><span>{{ t('music.albumSummary', { albums: filteredAlbums.length, songs: libraryMusic.length }) }}</span></div>
        <div class="album-grid">
          <button v-for="album in filteredAlbums" :key="album.id" class="album-card" :class="{ active: activeAlbum?.id === album.id }" @click="activeAlbumId = album.id">
            <span class="album-cover"><img v-if="albumCover(album)" :src="albumCover(album)" alt="" /><Disc3 v-else :size="30" /></span><strong>{{ album.title }}</strong><small>{{ album.artist }} · {{ t('music.songCount', { count: album.items.length }) }}</small>
          </button>
        </div>
      </section>
      ...
      <section v-if="activeAlbum" class="album-detail">
        ...
        <div class="track-list">
          <article v-for="(item, index) in activeAlbum.items" :key="item.path" class="track-row" :class="{ current: isCurrent(item) }">
    ...
    <div v-else-if="!sources.enabledSources.length" class="empty-state"><div><span class="empty-icon"><FolderCog :size="24" /></span><h2>{{ t('music.chooseFolder') }}</h2><p>{{ t('music.chooseFolderBody') }}</p><RouterLink to="/settings/library" class="primary-button">{{ t('music.configureFolder') }}</RouterLink></div></div>
    <div v-else-if="scanning" class="empty-state"><div><LoaderCircle class="spin" :size="28" /><h2>{{ t('music.building') }}</h2><p>{{ t('music.buildingBody') }}</p></div></div>
    <div v-else class="empty-state"><div><span class="empty-icon"><Music2 :size="24" /></span><h2>{{ t('music.empty') }}</h2><p>{{ t('music.formats') }}</p><button class="primary-button" @click="scanner.start"><RefreshCw :size="17" />{{ t('music.rescan') }}</button></div></div>
  </section>
</template>
